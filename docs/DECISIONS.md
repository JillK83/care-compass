...
| O21 | Supabase lint flagged "RLS Disabled in Public Entity" on care_compass.demand_signals — RLS was off despite O10 marking it "Resolved" on 2026-07-01 with RLS enabled. Root cause of the regression not identified (not investigated — out of scope for demo timeline). | Jillian | Resolved 2026-09-08 — re-enabled RLS via `ALTER TABLE care_compass.demand_signals ENABLE ROW LEVEL SECURITY`. Recreated policies matching original A07/O10 design: SELECT restricted to `authenticated` (qual: true), INSERT open to `anon` (with_check: true), no UPDATE/DELETE. Verified live via pg_policies query — matches spec exactly. End-to-end flow (Compass anon insert → Console authenticated read) not yet re-tested post-fix; recommend confirming before demo. |
